...
void __SetupUSBDescriptor() {
    if (!usbd_desc_cfg) {
         ...
         * -----    MIDI
         */
        bool install_MIDI = false;
#if CFG_TUD_MIDI
        install_MIDI = __USBInstallMIDI;
        uint8_t midi_desc[TUD_MIDI_DESC_LEN] = {
            // Interface number, string index, EP Out & EP In address, EP size
            TUD_MIDI_DESCRIPTOR(interface_count, 0, USBD_MIDI_EP_OUT, USBD_MIDI_EP_IN, CFG_TUD_MIDI_EP_BUFSIZE)
        };
        interface_count += (install_MIDI ? 2 : 0);
#else
        uint8_t midi_desc[0] = {};
#endif
        ...
        int usbd_desc_len =
            ...
            + (install_MIDI ? sizeof(midi_desc) : 0);
